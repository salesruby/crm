            ...
            <div id="all-todo" class="row">
                ...
                <div class="col-sm-6 col-xl-3">
                    ...
                    <div class="row to-do-list">
                        <span id="toggle-create-todo-form"class="col-md-12 text-center mini-stat-icon" data-toggle="collapse" data-target="#create-todo-form">
                    ...
                    <div class="row to-do-list">
                        <ul id="tomorrow-list">
                            @foreach($tomorrow_tasks as $tomorrow_task)
                                <li id="tomorrow_task{{$tomorrow_task->id}}" data-id="{{$tomorrow_task->id}}">{{$tomorrow_task->action}}</li>
                                @csrf
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div id="today-list-div" class="row to-do-list" style="background-color: red;">
                        <ul id="today-list">
                            @foreach($today_tasks as $today_task)
                                <li id="today_task{{$today_task->id}}" data-id="{{$today_task->id}}">{{$today_task->action}}</li>
                                @csrf
                            @endforeach
                        </ul>
                    </div>
